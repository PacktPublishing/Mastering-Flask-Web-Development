...
class AppointmentRepository: 
    ...
    async def insert_appt(self, appt: Appointment) -> bool: 
        try:
            ...
            sql = insert(Appointment).values(ticketid= appt.ticketid, patientid=appt.patientid, docid=appt.docid, priority_level=appt.priority_level, date_scheduled=datetime.strptime(appt.date_scheduled, '%Y-%m-%d').date(), time_scheduled=time.strptime(appt.date_scheduled, '%H:%M'),
    ...
    async def select_all_appt_doc(self, docid):
        sql = select(Appointment).where(Appointment.docid == docid)
        sql.execution_options(synchronize_session="fetch")
        q = await self.sess.execute(sql)
        records = q.scalars().all()
        await self.sess.close()
