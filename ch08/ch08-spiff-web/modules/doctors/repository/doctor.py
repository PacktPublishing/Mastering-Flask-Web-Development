...
class DoctorRepository: 
    ...
    async def insert_doctor(self, doc: Doctor) -> bool: 
        try:
            ...
            sql = insert(Doctor).values(empid=doc.empid, username=doc.username, role=doc.role, firstname=doc.firstname, midname=doc.midname, lastname=doc.lastname, 
                                        age=doc.age, email=doc.email, mobile=doc.mobile, position=doc.position, specialization=doc.specialization, status=doc.status, date_started=datetime.strptime(doc.date_started, '%Y-%m-%d').date())
    ...
    async def select_doc_specialization(self, specialization:str): 
        sql = select(Doctor).where(Doctor.specialization == specialization)
        sql.execution_options(synchronize_session="fetch")
        q = await self.sess.execute(sql)
        record = q.scalars().all()
        await self.sess.close()
