...
async def add_login():
    ...
    result = await repo.insert_login(login_json)
    if result == False:
...
async def get_login(username:str):
    repo = LoginRepository()
    result = await repo.select_login_username(username)
    return jsonify(records=result)
...
async def update_username():
    login_json = request.get_json()
    repo = LoginRepository()
    result = await repo.change_password(login_json)
    if result == False:
        return jsonify(message="error"), 500
    else:
        return jsonify(record=login_json)
...
async def delete_login(username:str):
    repo = LoginRepository()
    result = await repo.delete_login(username)
    if result == False:
        return jsonify(message="error"), 500
    else:
