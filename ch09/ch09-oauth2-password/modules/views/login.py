...
async def login_authorize():
    ...
    return oauth_server.create_authorization_response(grant_user=login[0])
...
@current_app.route('/oauth/revoke', methods=['POST'])
def revoke_token():
    return oauth_server.create_endpoint_response('revocation')
