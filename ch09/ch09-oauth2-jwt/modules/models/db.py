...
from sqlalchemy import  Column, Text ,ForeignKey, Integer, String, Date, DateTime, Sequence, Boolean, Float, Text, Double
...
class Client(Base, OAuth2ClientMixin):
    ...
    jwks = Column(Text(), nullable=False)
    
    
    login = relationship('Login', back_populates="client")
    
    def __init__(self, user_id, client_id, client_id_issued_at, client_secret, jwks, id=None):
      self.id = id
      self.user_id = user_id
