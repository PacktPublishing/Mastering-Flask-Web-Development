...
def get_token_refresh_task_wrapper(token_str):
     async def get_token_refresh_task(token_str):
        try:
            async with db_session() as sess:
               async with sess.begin(): 
                    repo = TokenRepository(sess)
                    records = await repo.select_token_refresh(token_str)
                    print(records[0])
                    #login_rec = [rec.to_json() for rec in records]
                    return records[0]
        except Exception as e:
            print(e)
            return None
     return run(get_token_refresh_task(token_str))
