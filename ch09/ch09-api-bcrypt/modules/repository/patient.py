...
class PatientRepository: 
    ...
    async def insert_patient(self, patient: Patient) -> bool: 
        try:
            ...
            sql = insert(Patient).values(patientid=patient.patientid, username=patient.username, firstname=patient.firstname, midname=patient.midname, lastname=patient.lastname, 
                                        birthday=datetime.strptime(patient.birthday, '%Y-%m-%d').date(), age=patient.age, address=patient.address, email=patient.email, mobile=patient.mobile, gender=patient.gender)
    ...
    async def delete_patient_username(self, username:str) -> bool: 
        try:
           sql = delete(Patient).where(Patient.username == username)
           await self.sess.execute(sql)
           await self.sess.commit()
           await self.sess.close()
           return True
