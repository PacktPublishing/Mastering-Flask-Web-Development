...
from sqlalchemy import update, delete, insert, and_
...
class LoginRepository: 
    ...
    async def insert_login(self, login: Login) -> bool: 
        try:
            sql = insert(Login).values(username=login.username, password=login.password, token=login.token, role=login.role)
    ...
    async def delete_login_username(self, username:str) -> bool: 
        try:
           sql = delete(Login).where(Login.username == username)
           await self.sess.execute(sql)
           await self.sess.commit()
           await self.sess.close()
           return True
    ...
    async def select_login_username(self, username:str): 
        sql = select(Login).where(Login.username == username)
        sql.execution_options(synchronize_session="fetch")
        q = await self.sess.execute(sql)
        record = q.scalars().all()
        await self.sess.close()
